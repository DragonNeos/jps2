...
import static com.jps2.util.Utilities.formatString;
...
public class PBP {
	...
	public String toString() {
		...
		str.append("p_magic "
				+ "\t\t"
				+ formatString("long", Long.toHexString(p_magic & 0xFFFFFFFFL)
						.toUpperCase()) + "\n");
		str.append("p_version "
				+ "\t\t"
				+ formatString("long", Long
						.toHexString(p_version & 0xFFFFFFFFL).toUpperCase())
				+ "\n");
		str
				.append("p_offset_param_sfo "
						+ "\t"
						+ formatString("long", Long.toHexString(
								p_offset_param_sfo & 0xFFFFFFFFL).toUpperCase())
						+ "\n");
		str
				.append("p_offset_icon0_png "
						+ "\t"
						+ formatString("long", Long.toHexString(
								p_offset_icon0_png & 0xFFFFFFFFL).toUpperCase())
						+ "\n");
		str
				.append("p_offset_icon1_pmf "
						+ "\t"
						+ formatString("long", Long.toHexString(
								p_offset_icon1_pmf & 0xFFFFFFFFL).toUpperCase())
						+ "\n");
		str.append("p_offset_pic0_png "
				+ "\t"
				+ formatString("long", Long.toHexString(
						p_offset_pic0_png & 0xFFFFFFFFL).toUpperCase()) + "\n");
		str.append("p_offset_pic1_png "
				+ "\t"
				+ formatString("long", Long.toHexString(
						p_offset_pic1_png & 0xFFFFFFFFL).toUpperCase()) + "\n");
		str.append("p_offset_snd0_at3 "
				+ "\t"
				+ formatString("long", Long.toHexString(
						p_offset_snd0_at3 & 0xFFFFFFFFL).toUpperCase()) + "\n");
		str.append("p_offset_psp_data "
				+ "\t"
				+ formatString("long", Long.toHexString(
						p_offset_psp_data & 0xFFFFFFFFL).toUpperCase()) + "\n");
		str
				.append("p_offset_psar_data "
						+ "\t"
						+ formatString("long", Long.toHexString(
								p_offset_psar_data & 0xFFFFFFFFL).toUpperCase())
						+ "\n");
		return str.toString();
	}
	...
	public static void unpackPBP(ByteBuffer f) throws IOException {
		...
		if (pbp.size_param_sfo > 0) {
			...
			f.get(data);
			FileOutputStream f1 = new FileOutputStream(PBP_UNPACK_PATH_PREFIX
					+ "param.sfo");
			f1.write(data);
			f1.close();
		}
