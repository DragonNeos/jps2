...
public class PluginConfigDialog extends JDialog {
	...
	public PluginConfigDialog() {
		...
		super(MainWindow.getInstance(), ResourceManager.getString("config.plugin.title"), true);
		// dispose this dialog if presse close button
		setDefaultCloseOperation(DISPOSE_ON_CLOSE);
		// set dialog icon
		setIconImage(ResourceManager.getIcon("/icons/16x16/config.png").getImage());
		// contruct dialog UI
		initComponents();
	...
	private final void initComponents() {
		...
		add(new JPanel(new FlowLayout(FlowLayout.TRAILING)) {
			{
				add(new JButton(new AbstractAction(ResourceManager.getString("config.plugin.ok.text")) {
				...
				add(new JButton(new AbstractAction(ResourceManager.getString("config.plugin.cancel.text")) {
