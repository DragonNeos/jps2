...
public class UmdIsoReader {
	...
	public UmdIsoReader(final String umdFilename) throws IOException,
	...
	public byte[] readSector(final int sectorNumber) throws IOException {
		...
		if (format == FileFormat.CompressedCSO) {
			...
			try {
				...
				final ByteArrayInputStream b = new ByteArrayInputStream(
						compressedData);
				inf.reset(b);
				// inf.setRawStream(b);
				inf.readAll(data, 0, data.length);
			} catch (final IOException e) {
				System.err.print("Exception while uncompressing sector from "
						+ fileName);
				e.printStackTrace();
			}
	...
	private Iso9660File getFileEntry(final String filePath) throws IOException,
			FileNotFoundException {
		Iso9660File info;
		...
		if (parentDirectoryIndex >= 0) {
			final String parentDirectory = filePath.substring(0,
					parentDirectoryIndex);
			final Iso9660Directory dir = dirCache.get(parentDirectory);
			if (dir != null) {
				final int index = dir.getFileIndex(filePath
	...
	public String[] listDirectory(final String filePath) throws IOException,
			FileNotFoundException {
	...
	public int getFileProperties(final String filePath) throws IOException,
			FileNotFoundException {
		if (filePath.compareTo("") == 0) {
			return 2;
		}
	...
	public boolean isDirectory(final String filePath) throws IOException,
			FileNotFoundException {
