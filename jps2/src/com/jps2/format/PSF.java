...
public class PSF {
	...
	public void read(ByteBuffer f) throws IOException {
		...
		if (ident != PSF_IDENT) {
			System.out.println("Not a valid PSF file (ident="
					+ String.format("%08X", ident) + ")");
			return;
		}
		...
		for (PSFKeyValuePair pair : pairList) {
			...
			case PSF_DATA_TYPE_STRING:
				...
				pair.data = new String(s, 0,
						s[s.length - 1] == '\0' ? s.length - 1 : s.length,
						"UTF-8");

				// System.out.println(String.format("offset=%08X key='%s' string '%s' [len=%d]",
				// keyTableOffset + pair.keyOffset, pair.key, pair.data,
				// pair.dataSize));
				break;
			...
			case PSF_DATA_TYPE_INT32:
				...
				// keyTableOffset + pair.keyOffset, pair.key, pair.data,
				// pair.data, pair.dataSize));
				break;

			default:
				System.out.println(String.format(
						"offset=%08X key='%s' unhandled data type %d [len=%d]",
						keyTableOffset + pair.keyOffset, pair.key,
						pair.dataType, pair.dataSize));
				break;
			}
		}
	...
	// assumes we want to write at the start of the buffer, and that the current
	// buffer position is 0
	// doesn't handle psfOffset
	public void write(ByteBuffer f) {
		if (indexEntryCount != pairList.size())
			throw new RuntimeException(
					"incremental size and actual size do not match! "
							+ indexEntryCount + "/" + pairList.size());
		...
		for (PSFKeyValuePair pair : pairList) {
			...
			// System.err.println("PSF write key   fp=" + f.position() +
			// " datalen=" + (pair.key.length() + 1) + " top=" + (f.position() +
			// pair.key.length() + 1));
			writeStringZ(f, pair.key);

			f.position(valueTableOffset + pair.valueOffset);
			// System.err.println("PSF write value fp=" + f.position() +
			// " datalen=" + (pair.dataSizePadded) + " top=" + (f.position() +
			// pair.dataSizePadded));
			switch (pair.dataType) {
			case PSF_DATA_TYPE_BINARY:
			...
			default:
				System.out.println("not writing unhandled data type "
						+ pair.dataType);
				break;
			}
		}
	...
	public void put(String key, byte[] data) {
		PSFKeyValuePair pair = new PSFKeyValuePair(key, PSF_DATA_TYPE_BINARY,
				data.length, data);
		pairList.add(pair);
	...
	public void put(String key, String data, int rawlen) {
		...
		// System.out.println("put string '" + data + "' size mismatch. UTF-8="
		// + b.length + " regular=" + (data.length() + 1));

		// PSFKeyValuePair pair = new PSFKeyValuePair(key, PSF_DATA_TYPE_STRING,
		// data.length() + 1, rawlen, data);
		PSFKeyValuePair pair = new PSFKeyValuePair(key, PSF_DATA_TYPE_STRING,
				b.length + 1, rawlen, data);
		pairList.add(pair);
	...
	public void put(String key, int data) {
		PSFKeyValuePair pair = new PSFKeyValuePair(key, PSF_DATA_TYPE_INT32, 4,
				data);
		pairList.add(pair);
	...
	private void calculateTables() {
		...
		// position the key table after the index table and before the value
		// table
		// 20 byte PSF header
	...
	private void calculateSize() {
		...
		for (PSFKeyValuePair pair : pairList) {
			int keyHighBound = keyTableOffset + pair.keyOffset
					+ pair.key.length() + 1;
			int valueHighBound = valueTableOffset + pair.valueOffset
					+ pair.dataSizePadded;
	...
	public String toString() {
		 ...
		 * ident, ident)); sb.append(String.format("version 0x%08X %d\n",
		 * version, version));
	...
	public boolean isLikelyHomebrew() {
		...
		Integer bootable = (Integer) get("BOOTABLE"); // don't use getNumeric,
		...
		if (safeEquals(disc_version, "1.00")
				...
				&& // loco roco demo, should not false positive since that demo
					// has sys ver 3.40
				safeEquals(category, "MG") && safeEquals(bootable, ref_one)
		...
		} else if (indexEntryCount == 4 && safeEquals(category, "MG")
	...
	public class PSFKeyValuePair {
		...
		public PSFKeyValuePair(String key, int dataType, int dataSize,
				Object data) {
			this(key, dataType, dataSize, dataSize + (dataSize & 3), data);
		}

		public PSFKeyValuePair(String key, int dataType, int dataSize,
				int dataSizePadded, Object data) {
			this.key = key;
		...
		/**
		 * only writes the index entry, since this class has doesn't know about
		 * the psf/key/value offsets
		 */
		public void write(ByteBuffer f) {
			// index table entry
		...
		public String toString() {
			 ...
			 * keyOffset)); sb.append(String.format("unknown1 0x%08X %d\n",
			 * unknown1, unknown1));
			 * sb.append(String.format("dataType 0x%08X %d\n", dataType,
			 * dataType)); sb.append(String.format("dataSize 0x%08X %d\n",
			 * dataSize, dataSize));
			...
			// sb.append(String.format("[offset=%08X] '%s' = [offset=%08X,len=%d,rawlen=%d] '"
			// + data + "'",
