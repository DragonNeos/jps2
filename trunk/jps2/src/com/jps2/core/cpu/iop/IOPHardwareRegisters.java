...
public final class IOPHardwareRegisters extends FastMemory {
	...
	@Override
	public void copy(final Memory memory, final int positionSource,
			final int positionDest) {
		throw new RuntimeException(
				"Unsuported Operation (copy is not allowed here (IOP)).");
	}
	...
	public int read32(int address) {
		...
		case 0x1060:
			logger.debug(String
					.format("RAM size read %x", super.read32(0x1060)));
			return super.read32(0x1060);
		case 0x1070:
			logger.debug(String.format("IREG 32bit read %x", super
					.read32(0x1070)));
			return super.read32(0x1070);
		case 0x1074:
			logger.debug(String.format("IMASK 32bit read %x", super
					.read32(0x1074)));
			return super.read32(0x1074);
		case 0x1078:
			logger.debug(String.format("ICTRL 32bit read %x", super
					.read32(0x1078)));
			hard = super.read32(0x1078);
			super.write32(0x1078, 0);
			return hard;
		case 0x10a0:
			logger.debug(String.format("DMA2 MADR 32bit read %x", super
					.read32(0x10a0)));
			return HWx_DMA2_MADR;
		case 0x10a4:
			logger.debug(String.format("DMA2 BCR 32bit read %x", super
					.read32(0x10a4)));
			return HWx_DMA2_BCR;
		case 0x10a8:
			logger.debug(String.format("DMA2 CHCR 32bit read %x", super
					.read32(0x10a8)));
			return HWx_DMA2_CHCR;

		case 0x10b0:
			logger.debug(String.format("DMA3 MADR 32bit read %x", super
					.read32(0x10b0)));
			return HWx_DMA3_MADR;
		case 0x10b4:
			logger.debug(String.format("DMA3 BCR 32bit read %x", super
					.read32(0x10b4)));
			return HWx_DMA3_BCR;
		case 0x10b8:
			logger.debug(String.format("DMA3 CHCR 32bit read %x", super
					.read32(0x10b8)));
			return HWx_DMA3_CHCR;

		case 0x1520:
			logger.debug(String
					.format("DMA9 MADR 32bit read %x", HWx_DMA9_MADR));
		...
		case 0x1528:
			logger.debug(String
					.format("DMA9 CHCR 32bit read %x", HWx_DMA9_CHCR));
			return HWx_DMA9_CHCR;
		case 0x152C:
			logger.debug(String
					.format("DMA9 TADR 32bit read %x", HWx_DMA9_TADR));
			return HWx_DMA9_TADR;

		case 0x1530:
			logger.debug(String.format("DMA10 MADR 32bit read %x",
					HWx_DMA10_MADR));
			return HWx_DMA10_MADR;
		case 0x1534:
			logger.debug(String
					.format("DMA10 BCR 32bit read %x", HWx_DMA10_BCR));
			return HWx_DMA10_BCR;
		case 0x1538:
			logger.debug(String.format("DMA10 CHCR 32bit read %x",
					HWx_DMA10_CHCR));
		...
		case 0x1008:
			...
			logger
					.debug(String.format("SSBUS <spd_delay> 32bit read %x",
							hard));
			return hard;
		case 0x100C:
		...
		case 0x101C:
			...
			logger
					.debug(String.format("SSBUS <pio_delay> 32bit read %x",
							hard));
			return hard;
		case 0x1020:
		...
		case 0x1410:
			...
			logger.debug(String
					.format("SSBUS <dev9_addr3> 32bit read %x", hard));
			return hard;
		case 0x1414:
		...
		case 0x1418:
			...
			logger.debug(String.format("SSBUS <dev9_delay2> 32bit read %x",
					hard));
			return hard;
		case 0x141C:
			hard = super.read32(0x141C);
			logger.debug(String.format("SSBUS <dev9_delay3> 32bit read %x",
					hard));
			return hard;
		case 0x1420:
			hard = super.read32(0x1420);
			logger.debug(String.format("SSBUS <dev9_delay1> 32bit read %x",
					hard));
			return hard;
		...
		case 0x10c8:
			logger.debug(String
					.format("DMA4 CHCR 32bit read %x", HWx_DMA4_CHCR));
		...
		case 0x1508:
			logger.debug(String
					.format("DMA7 CHCR 32bit read %x", HWx_DMA7_CHCR));
		...
		default:
			hard = super.read32(address);
			logger.debug(String.format("*Unknown 32bit read at address %x: %x",
					address, hard));
			return hard;
		}
	}
	...
	public void write16(int address, final short data) {
		...
		if ((address & 0xf) == 0x9)
			logger.debug(String.format(
					"16bit write (possible chcr set) %x value %x", address,
					data));
		...
		case 0x1040:
			...
			logger.debug(String.format("sio write16 %x, %x", address & 0xf,
					data));
			return;
		case 0x1044:
			logger.debug(String.format("sio write16 %x, %x", address & 0xf,
		...
		default:
			...
			super.write16(address, data);
			logger.debug(String.format(
					"*Unknown 16bit write at address %x value %x", address,
					data));
			return;
		}
	...
	public void write32(int address, final int data) {
		...
		case 0x1040:
			...
			Emulator.getInstance().getSio().sioWrite8(
					(byte) ((data & 0xff00) >> 8));
			Emulator.getInstance().getSio().sioWrite8(
		...
		case 0x1000:
			super.write32(address, data);
			logger
					.debug(String.format("SSBUS <spd_addr> 32bit write %x",
							data));
			return;
		case 0x1004:
			super.write32(address, data);
			logger
					.debug(String.format("SSBUS <pio_addr> 32bit write %x",
							data));
			return;
		case 0x1008:
			super.write32(address, data);
			logger.debug(String
					.format("SSBUS <spd_delay> 32bit write %x", data));
			return;
		case 0x100C:
			super.write32(address, data);
			logger
					.debug(String.format("SSBUS dev1_delay 32bit write %x",
							data));
			return;
		case 0x1010:
			super.write32(address, data);
		...
		case 0x1018:
			super.write32(address, data);
			logger
					.debug(String.format("SSBUS dev5_delay 32bit write %x",
							data));
			return;
		case 0x101C:
			super.write32(address, data);
			logger.debug(String
					.format("SSBUS <pio_delay> 32bit write %x", data));
			return;
		case 0x1020:
			super.write32(address, data);
		...
		case 0x1410:
			super.write32(address, data);
			logger.debug(String.format("SSBUS <dev9_addr3> 32bit write %x",
					data));
			return;
		case 0x1414:
			super.write32(address, data);
			logger
					.debug(String.format("SSBUS spu1_delay 32bit write %x",
							data));
			return;
		case 0x1418:
			super.write32(address, data);
			logger.debug(String.format("SSBUS <dev9_delay2> 32bit write %x",
					data));
			return;
		case 0x141C:
			super.write32(address, data);
			logger.debug(String.format("SSBUS <dev9_delay3> 32bit write %x",
					data));
			return;
		case 0x1420:
			super.write32(address, data);
			logger.debug(String.format("SSBUS <dev9_delay1> 32bit write %x",
					data));
			return;

			// ------------------------------------------------------------------
			// case 0x1080:
			// logger.debug(String.format("DMA0 MADR 32bit write %x", data));
			// HW_DMA0_MADR = data;
			// return; // DMA0 madr
			// case 0x1084:
			...
			// return; // DMA0 bcr
			// case 0x1088:
			// logger.debug(String.format("DMA0 CHCR 32bit write %x", data));
			...
			// ------------------------------------------------------------------
			// case 0x1090:
			// logger.debug(String.format("DMA1 MADR 32bit write %x", data));
			// HW_DMA1_MADR = data;
			// return; // DMA1 madr
			// case 0x1094:
			...
			// return; // DMA1 bcr
			// case 0x1098:
			// logger.debug(String.format("DMA1 CHCR 32bit write %x", data));
			...
			// ------------------------------------------------------------------
			// case 0x10a0:
			// logger.debug(String.format("DMA2 MADR 32bit write %x", data));
			// HW_DMA2_MADR = data;
			// return; // DMA2 madr
			// case 0x10a4:
			...
			// return; // DMA2 bcr
			// case 0x10a8:
			// logger.debug(String.format("DMA2 CHCR 32bit write %x", data));
			...
			// ------------------------------------------------------------------
			// case 0x10b0:
			// logger.debug(String.format("DMA3 MADR 32bit write %x", data));
			// HW_DMA3_MADR = data;
			// return; // DMA3 madr
			// case 0x10b4:
			...
			// return; // DMA3 bcr
			// case 0x10b8:
			// logger.debug(String.format("DMA3 CHCR 32bit write %x", data));
			...
			// ------------------------------------------------------------------
			// case 0x10c0:
			// logger.debug(String.format("DMA4 MADR 32bit write %x", data));
			...
			// return; // DMA4 bcr
			// case 0x10c8:
			// logger.debug(String.format("DMA4 CHCR 32bit write %x", data));
			...
			//
			// case 0x10e0:
			// logger.debug(String.format("DMA6 MADR 32bit write %x", data));
			// HW_DMA6_MADR = value;
			// return; // DMA6 madr
			// case 0x10e4:
			...
			// return; // DMA6 bcr
			// case 0x10e8:
			// logger.debug(String.format("DMA6 CHCR 32bit write %x", data));
			...
			// ------------------------------------------------------------------
			// case 0x1500:
			// logger.debug(String.format("DMA7 MADR 32bit write %x", data));
			...
			// return; // DMA7 bcr
			// case 0x1508:
			// logger.debug(String.format("DMA7 CHCR 32bit write %x", data));
			...
			// ------------------------------------------------------------------
			// case 0x1510:
			// logger.debug(String.format("DMA8 MADR 32bit write %x", data));
			// HW_DMA8_MADR = data;
			// return; // DMA8 madr
			// case 0x1514:
			...
			// return; // DMA8 bcr
			// case 0x1518:
			// logger.debug(String.format("DMA8 CHCR 32bit write %x", data));
			...
			// ------------------------------------------------------------------
			// case 0x1520:
			// logger.debug(String.format("DMA9 MADR 32bit write %x", data));
			// HW_DMA9_MADR = data;
			// return; // DMA9 madr
			// case 0x1524:
			...
			// return; // DMA9 bcr
			// case 0x1528:
			// logger.debug(String.format("DMA9 CHCR 32bit write %x", data));
			// HW_DMA9_CHCR = data; // DMA9 chcr (SIF0)
			// DmaExec2(9);
			// return;
			// case 0x152c:
			// logger.debug(String.format("DMA9 TADR 32bit write %x", data));
			// HW_DMA9_TADR = data;
			// return; // DMA9 tadr
			//
			// //
			// ------------------------------------------------------------------
			// case 0x1530:
			// logger.debug(String.format("DMA10 MADR 32bit write %x", data));
			// HW_DMA10_MADR = data;
			// return; // DMA10 madr
			// case 0x1534:
			// logger.debug(String.format("DMA10 BCR 32bit write %x", data));
			// HW_DMA10_BCR = data;
			// return; // DMA10 bcr
			// case 0x1538:
			// logger.debug(String.format("DMA10 CHCR 32bit write %x", data));
		...
		case 0x10f0:
			...
			// * logger.debug(String.format("GPU STATUS 32bit write %x", value);
		...
		case 0x1450:
			 ...
			 * 0x20000 || psxSu32(0x30) == 0x40000)) { // don't ask me why :P
			 * psxSu32(0x20) = 0x10000; psxSu32(0x30) = 0x10000; }
			 */
			...
			// case 0x8238:
			// case 0x823C:
			// logger.debug(String.format("SIO2 write param[%d] <- %x", (address
			// - 8200) / 4, data));
			...
			// case 0x8250:
			// case 0x8258:
			// logger.debug(String.format("SIO2 write send1[%d] <- %x", (address
			// - 8240) / 8, data));
			...
			// case 0x8254:
			// case 0x825C:
			// logger.debug(String.format("SIO2 write send2[%d] <- %x", (address
			// - 8244) / 8, data));
		...
		default:
			super.write32(address, data);
			logger.debug(String.format(
					"*Unknown 32bit write at address %x value %x", address,
					data));
			return;
		}
	}
	...
	public void write8(int address, final byte data) {
		...
		if ((address & 0xf) == 0xa) {
			logger.debug(String.format(
					"8bit write (possible chcr set) to addr 0x%x = 0x%x",
					address, data));
		}
		...
		case IOP_T5_TARGET:
			logger.debug(String.format(
					"IOP Counter Write8 to addr 0x%x = 0x%x", address, data));
		...
		case 0x380c: {
			...
			// case 0x8260:
			// logger.debug(String.format("SIO2 write8 DATAIN <- %08X", data));
			// sio2_serialIn(data);
		...
		default:
			super.write8(address, data);
			logger.debug(String
					.format("*Unknown 8bit write at address %x value %x",
							address, data));
			return;
		}
	}
